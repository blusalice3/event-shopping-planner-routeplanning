...
export const normalizePrice = (
  price: ShoppingItem["price"] | -1,
): ShoppingItem["price"] => (isUndefinedPrice(price) ? null : price);

export const getSafePriceForCalculation = (
  price: ShoppingItem["price"] | -1,
): number => (isUndefinedPrice(price) ? 0 : (price ?? 0));
...
export const getChargeableQuantity = (
  ...
  if (item.purchaseStatus === "Purchased") return getPlannedQuantity(item);
  if (item.purchaseStatus === "LimitedPurchase")
    return getActualPurchasedQuantity(item) ?? 0;
  return 0;
};

export const getPlannedBudgetQuantity = (
  item: Pick<ShoppingItem, "quantity">,
): number => getPlannedQuantity(item);
...
export const hasMissingLimitedPurchaseQuantity = (
  ...
  item.purchaseStatus === "LimitedPurchase" &&
  getActualPurchasedQuantity(item) === undefined;
...
export const matchesPurchaseStatusFilter = (
  ...
  if (filterStatus === "LimitedPurchase")
    return item.purchaseStatus === "LimitedPurchase";
...
export const validateLimitedPurchaseQuantities = (
  ...
  if (!Number.isInteger(actual))
    return { ok: false, error: "actual_not_integer" };
  ...
  if (!Number.isInteger(planned))
    return { ok: false, error: "planned_not_integer" };
  if (planned < 1) return { ok: false, error: "planned_not_positive" };
  if (actual >= planned)
    return { ok: false, error: "actual_not_less_than_planned" };
  return { ok: true };
};
...
export const applyLimitedPurchase = (
  ...
  return {
    ...
    ...(input.actual !== undefined
      ? { limitedPurchasedQuantity: input.actual }
      : {}),
  };
};
...
export const normalizeLimitedPurchaseFields = (
  item: ShoppingItem,
): ShoppingItem => {
  const planned = getPlannedQuantity(item);
...
export const getNextPurchaseStatus = (
  ...
  const nextStatus =
    PurchaseStatuses[(currentIndex + 1) % PurchaseStatuses.length];

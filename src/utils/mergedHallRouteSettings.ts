...
export function buildMergedHallRouteSettings(params: {
  ...
  mergedHalls: HallDefinition[];
  mergedSettings: HallRouteSettings;
  ...
  const mapHalls = hasMap
    ? hallDefinitionsStore[eventName]?.[mapTabName!] || []
    : [];
  const maplessKey = dayName ? getMaplessKey(dayName) : null;
  const maplessHalls = maplessKey
    ? hallDefinitionsStore[eventName]?.[maplessKey] || []
    : [];
  const mergedHalls = [...mapHalls, ...maplessHalls];

  const dayMapData = hasMap
    ? mapDataStore[eventName]?.[mapTabName!] || null
    : null;

  const mapSettings = hasMap
    ? hallRouteSettingsStore[eventName]?.[mapTabName!]
    : undefined;
  const maplessSettings = maplessKey
    ? hallRouteSettingsStore[eventName]?.[maplessKey]
    : undefined;
  ...
  if (executeIds.length > 0) {
    ...
    executeIds.forEach((itemId) => {
      ...
      if (!item) return;
      const priority = (item.priorityLevel || "none") as
        | "none"
        | "priority"
        | "highest";

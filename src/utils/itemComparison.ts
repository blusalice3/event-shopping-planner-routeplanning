...
export function getItemKeyWithoutTitle(
  item: ShoppingItem | Omit<ShoppingItem, "id" | "purchaseStatus">,
): string {
...
export function insertItemSorted(
  ...
  const sameDayItems = newItems.filter(
    (item) => item.eventDate === newItem.eventDate,
  );
  ...
  for (let i = 0; i < sameDayItems.length; i++) {
    ...
    if (
      currentKey.localeCompare(newItemKey, "ja", {
        numeric: true,
        sensitivity: "base",
      }) > 0
    ) {
  ...
  const sameDayStartIndex = newItems.findIndex(
    (item) => item.eventDate === newItem.eventDate,
  );
  const actualInsertIndex =
    sameDayStartIndex === -1
      ? newItems.length
      : sameDayStartIndex + insertIndex;

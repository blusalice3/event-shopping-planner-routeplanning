...
describe('purchaseQuantity utilities', () => {
  it('formats limited purchase quantities', () => {
    ...
    expect(formatDisplayQuantity(makeItem({ purchaseStatus: 'LimitedPurchase', quantity: 5 }))).toBe(
      '-/5',
    );
  });

  it('identifies purchased-like and price-required statuses', () => {
    expect(isPurchasedLike(makeItem({ purchaseStatus: 'Purchased' }))).toBe(true);
    expect(isPurchasedLike(makeItem({ purchaseStatus: 'LimitedPurchase' }))).toBe(true);
    expect(isPriceRequiredStatus(makeItem({ purchaseStatus: 'Purchased' }))).toBe(true);
    expect(isPriceRequiredStatus(makeItem({ purchaseStatus: 'LimitedPurchase' }))).toBe(true);
  });
  ...
  it('uses actual quantity for limited purchase charge calculations', () => {
    ...
    expect(getChargeableQuantity(makeItem({ purchaseStatus: 'LimitedPurchase', quantity: 5 }))).toBe(
      0,
    );
    expect(
      getActualPurchasedQuantity(
  ...
  it('detects missing limited purchase quantities from invalid actual values', () => {
    expect(hasMissingLimitedPurchaseQuantity(makeItem({ purchaseStatus: 'LimitedPurchase' }))).toBe(
      true,
    );
    expect(
      hasMissingLimitedPurchaseQuantity(
  ...
  it('counts purchased and limited purchase items correctly', () => {
    expect(isCountedAsPurchased(makeItem({ purchaseStatus: 'Purchased' }))).toBe(true);
    expect(
      isCountedAsPurchased(
        makeItem({
          purchaseStatus: 'LimitedPurchase',
    ...
    expect(isCountedAsPurchased(makeItem({ purchaseStatus: 'LimitedPurchase', quantity: 5 }))).toBe(
      false,
    );
  });

  it('matches purchase status filters with Purchased and LimitedPurchase separated', () => {
    expect(matchesPurchaseStatusFilter(makeItem({ purchaseStatus: 'Purchased' }), 'Purchased')).toBe(
      true,
    );
    expect(
      matchesPurchaseStatusFilter(makeItem({ purchaseStatus: 'LimitedPurchase' }), 'Purchased'),
    ).toBe(false);
    expect(
      matchesPurchaseStatusFilter(makeItem({ purchaseStatus: 'LimitedPurchase' }), 'LimitedPurchase'),
    ).toBe(true);
  });
  ...
  it('counts limited purchase totals and missing quantities', () => {
    expect(
      getLimitedPurchaseCounts([
        makeItem({ id: '1', purchaseStatus: 'LimitedPurchase', quantity: 5, limitedPurchasedQuantity: 2 }),
        makeItem({ id: '2', purchaseStatus: 'LimitedPurchase', quantity: 5 }),
        makeItem({ id: '3', purchaseStatus: 'LimitedPurchase', quantity: 5, limitedPurchasedQuantity: 1 }),
  ...
  it('normalizes stale or invalid limited purchase fields', () => {
    ...
    expect(normalizeLimitedPurchaseFields(makeItem({ price: -1 })).price).toBeNull();
  });

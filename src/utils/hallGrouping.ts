 ...
 * - '<hallId>' → ホールのみ（優先度なし）
 */
export function parseGroupId(groupId: string | null): {
  hallId: string | null;
...
export function buildItemRoutingSignature(
  items: ShoppingItem[],
  itemIds: string[],
): string {
...
export function isRouteCellResolvableOnMap(
  ...
  return mapData.blocks.some((block) =>
    block.numberCells.some(
      (numberCell) =>
        numberCell.row === cell.row && numberCell.col === cell.col,
    ),
  );
}
...
export function getMapRouteGroupParts(groupKey: string | null): {
  ...
  const normalizedHallId =
    hallId === undefined || hallId === "undefined" ? null : hallId;
  return {
...
export function areMapRouteGroupKeysCompatible(
  ...
  return pendingGroupKeys.every((groupKey) => {
    ...
    return (
      pending.hallId === anchor.hallId && pending.priority === anchor.priority
    );
  });
}
...
export function sortItemsByHallOrder(
  ...
  const grouped = groupItemsByHallOrder(
    items,
    dayMapData,
    hallDefinitions || [],
    hallOrder,
...
export function groupItemsByHallOrderWithResolver(
  ...
  if (hallDefinitions.length === 0) {
    ...
    const emit = (
      gId: "undefined" | "undefined:priority" | "undefined:highest",
    ) => {
      const bucket =
        gId === "undefined:highest"
          ? highestBucket
          : gId === "undefined:priority"
            ? priorityBucket
            : noneBucket;
      ...
      return {
        ...
        hallId: null,
        hallName: null,
        priority: (gId === "undefined:highest"
          ? "highest"
          : gId === "undefined:priority"
            ? "priority"
    ...
    (["undefined:highest", "undefined:priority", "undefined"] as const).forEach(
      (gId) => {
        if (emitted.has(gId)) return;
        const g = emit(gId);
        if (g) {

  ...
  return (
    <>
      ...
      <output data-testid="current-visit-id">
        {navigator.registration?.entries[navigator.registration.currentIndex]?.id ?? ''}
      </output>
      <output data-testid="registration-id">{navigator.registration?.id ?? ''}</output>
      <output data-testid="history-depth">{navigator.history.length}</output>
      <output data-testid="inspect-state">{String(navigator.isInspecting)}</output>
      ...
      <button type="button" onClick={() => void navigator.navigate(1, 'inspect')}>
        確認移動
      </button>
    </>
  );
...
describe('ShoppingList execution space navigator integration', () => {
  ...
  it('builds visits from the visible list and preserves temporary history across updates', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('history-depth')).toHaveTextContent('1'));

    fireEvent.click(screen.getByRole('button', { name: '表示更新' }));
    await waitFor(() => expect(screen.getByText('タイトルb更新')).toBeInTheDocument());
  ...
  it('re-resolves a surviving target by visit id when filtering changes its index during navigation', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('entry-count')).toHaveTextContent('3'));
    fireEvent.click(screen.getByRole('button', { name: '一時移動' }));
    ...
    await waitFor(() =>
      expect(screen.getByTestId('current-visit-id')).toHaveTextContent('B-02a:none'),
    );
  ...
  it('does not restore a deleted target or leave currentIndex out of range when filtering changes during navigation', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('entry-count')).toHaveTextContent('2'));
    await waitFor(() =>
      expect(screen.getByTestId('current-visit-id')).toHaveTextContent('C-03a:none'),
    );
    expect(screen.getByTestId('current-index')).toHaveTextContent('1');
    expect(screen.getByTestId('formal-index')).toHaveTextContent('0');
    await waitFor(() => expect(screen.getByTestId('history-depth')).toHaveTextContent('0'));
  ...
  it('resets navigation refs and target lock when the registration id changes', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('registration-id')).toHaveTextContent('execute:Day1'),
    );
    fireEvent.click(screen.getByRole('button', { name: '一時移動' }));
    expect(screen.getByTestId('current-index')).toHaveTextContent('1');
    fireEvent.click(screen.getByRole('button', { name: 'Day2へ切替' }));
    await waitFor(() =>
      expect(screen.getByTestId('registration-id')).toHaveTextContent('execute:Day2'),
    );
    await waitFor(() => expect(screen.getByTestId('history-depth')).toHaveTextContent('0'));
    expect(screen.getByTestId('temporary-mode')).toHaveTextContent('none');
    expect(screen.getByTestId('current-index')).toHaveTextContent('0');
    expect(screen.getByTestId('formal-index')).toHaveTextContent('0');
    expect(screen.getByTestId('current-visit-id')).toHaveTextContent('A-01a:none');
  });
  ...
  it('keeps navigator visits in the same hall and priority order as rendered space groups', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('entry-order')).toHaveTextContent(expectedOrder),
    );
  ...
  it('makes inspect mode read-only while keeping links and heading collapse available', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('inspect-state')).toHaveTextContent('true'));

    const targetCard = screen.getByText('タイトルb').closest('[data-item-id="b"]');
    expect(targetCard).not.toBeNull();
    const card = targetCard as HTMLElement;
    expect(card).toHaveAttribute('draggable', 'false');
    expect(within(card).getByRole('checkbox')).toBeDisabled();
    expect(within(card).getByRole('button', { name: /Current status/i })).toBeDisabled();
    expect(within(card).getByRole('link', { name: 'URLを開く' })).toHaveAttribute(
      'href',
    ...
    const heading = document.querySelector<HTMLElement>('[data-space-group-key="B-02a"]');

...
const ShoppingList: React.FC<ShoppingListProps> = ({
  ...
  const assignedMemberMap = useMemo(() => {
    ...
    for (const m of sharing.members) {
      map.set(String(m.jerseyNumber), { displayName: m.displayName, color: m.color });
    }
    return map;
  }, [sharing?.activeRoom, sharing?.members]);

  // 自分の背番号文字列（assignedTo比較用）
  const myJerseyStr = useMemo(() => {
    if (!sharing?.userId || !sharing.members.length) return null;
    const me = sharing.members.find((m) => m.userId === sharing.userId);
    return me ? String(me.jerseyNumber) : null;
  }, [sharing?.userId, sharing?.members]);
  ...
  const handleAssign = useCallback(async (targetJerseyNumber: number | null) => {
    ...
    const item = items.find((i) => i.id === itemId);
    if (item) {
      onUpdateItem({ ...item, assignedTo: targetJerseyNumber != null ? String(targetJerseyNumber) : undefined });
    }
  ...
  const handleTransfer = useCallback(async (
    ...
    for (const { itemId, quantity } of transferItems) {
      const item = items.find((i) => i.id === itemId);
      if (item) {
        onUpdateItem({ ...item, assignedTo: String(targetJerseyNumber), quantity });
        await sharing.assignItem(itemId, targetJerseyNumber);

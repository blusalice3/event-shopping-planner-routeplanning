...
export function useFocusSessionState(resumeState: FocusModeSessionState | null) {
  const [currentPhase, setCurrentPhase] = useState<FocusPhase>(() => resumeState?.phase || 'normal');
  ...
  const [phaseChangeDialog, setPhaseChangeDialog] = useState<PhaseChangeDialogState>({
    isOpen: false,
    targetPhase: null,
    hasSavedIndex: false,
    savedIndex: 0,

...
const mixedPhaseVisits: Record<
  "normal" | "postponed" | "late",
  ResumeVisitGroup[]
> = {
  normal: [makeVisit("2026-01-01-A-01a-none")],
  postponed: [
    makeVisit("2026-01-01-P-03a-none", "P", "03a", "postponed circle"),
  ],
  late: [],
};
...
describe("buildResumeChoiceDialogState", () => {
  ...
  it("lpc の visitKey が現在の並びに見つからない → lastChangeEnabled=false", () => {
    ...
    expect(result!.lastChangeEnabled).toBe(false);
    expect(result!.lastSpaceLabel).toBe(
      "対象スペースが現在の並びに見つかりません",
    );
  });
...
describe("resolveResumeChoice", () => {
  ...
  it("pointer + wasCompleted=false → isCompleted なし", () => {
    const r = resolveResumeChoice("pointer", {
      ...baseDialog,
      wasCompleted: false,
    });
  ...
  it("lastChange + disabled → 空", () => {
    const r = resolveResumeChoice("lastChange", {
      ...baseDialog,
      lastChangeEnabled: false,
    });
    expect(r).toEqual({});
  });
  ...
  it("phaseStart + disabled → 空", () => {
    const r = resolveResumeChoice("phaseStart", {
      ...baseDialog,
      phaseStartEnabled: false,
    });
    expect(r).toEqual({});
  });
  ...
  it("normalStart + disabled → 空", () => {
    const r = resolveResumeChoice("normalStart", {
      ...baseDialog,
      normalStartEnabled: false,
    });
    expect(r).toEqual({});
  });
});

...
export function PhaseChangeDialogView({
  ...
  const targetPhaseName =
    dialog.targetPhase === 'normal' ? '通常' : dialog.targetPhase === 'postponed' ? '後回し' : '遅参';
              ...
              <div className="space-y-2">
                ...
                {dialog.hasSavedIndex && (
                  ...
                  >
                    途中から再開
                    <span className="block text-xs opacity-80 mt-0.5">
                      {savedVisitInfo} （{dialog.savedIndex + 1}/{targetVisits.length}）
                    </span>
                  </button>
                )}
              </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-2xl max-w-sm w-full mx-4 overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-700">
          <h3 className="font-semibold text-slate-900 dark:text-white">
            {state.blockName}-{state.number} {state.items.length > 0 ? `（${state.items.length}件）` : ''}
          </h3>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-2xl max-w-lg w-full mx-4 overflow-hidden max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <TextField label="サークル名" value={form.circle} onChange={(value) => setForm((prev) => ({ ...prev, circle: value }))} required list="focus-add-circle-suggestions" placeholder="サークル名">
              {circles.length > 0 && (
            ...
            <TextField label="タイトル" value={form.title} onChange={(value) => setForm((prev) => ({ ...prev, title: value }))} placeholder="新刊セット" />
          </div>

          <div className="grid grid-cols-3 gap-4">
            <TextField label="参加日" value={dialog.eventDate} readOnly />
            <TextField label="ブロック" value={dialog.block} readOnly />
            <TextField label="ナンバー" value={dialog.number} onChange={(value) => setDialog((prev) => ({ ...prev, number: value }))} placeholder="01a" />
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
            <div className="relative">
              <label className={labelClass}>頒布価格</label>
              <input type="text" value={form.price} onChange={onPriceInputChange} className={`${formInputClass} pr-12`} placeholder="0" inputMode="numeric" />
              <span className="absolute right-3 top-9 text-slate-500 dark:text-slate-400">円</span>
            </div>
            <div>
              <label className={labelClass}>クイック選択</label>
              <select onChange={onPriceSelectChange} className={formInputClass} value={priceOptions.includes(Number(form.price)) ? form.price : ''}>
                <option value="" disabled>金額を選択...</option>
                {priceOptions.map((price) => (
                  <option key={price} value={price}>{price.toLocaleString()}円</option>
                ))}
              </select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <SelectField label="数量" value={form.quantity} onChange={(value) => setForm((prev) => ({ ...prev, quantity: value }))} options={Array.from({ length: 10 }, (_, i) => String(i + 1))} />
            <SelectField
              label="購入状態"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <TextField label="備考" value={form.remarks} onChange={(value) => setForm((prev) => ({ ...prev, remarks: value }))} placeholder="スケブお願い" />
            <TextField label="URL" value={form.url} onChange={(value) => setForm((prev) => ({ ...prev, url: value }))} placeholder="https://example.com" />
          </div>
        </div>
        <div className="p-4 border-t border-slate-200 dark:border-slate-700 flex gap-2">
          <button onClick={onClose} className="flex-1 py-2 px-4 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-300 rounded-lg font-medium transition-colors">
            キャンセル
          </button>
          <button onClick={onSubmit} disabled={!form.circle.trim()} className="flex-1 py-2 px-4 bg-green-600 hover:bg-green-700 disabled:bg-slate-400 text-white rounded-lg font-medium transition-colors">
            リストに追加
          </button>
        </div>
      </div>
  ...
  return (
    <div>
      ...
      <input
        ...
        className={readOnly ? `${formInputClass} bg-slate-100 dark:bg-slate-700` : formInputClass}
        readOnly={readOnly}
        list={list}

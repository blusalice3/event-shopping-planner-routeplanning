...
const CellItemsPopup: React.FC<CellItemsPopupProps> = ({
  ...
  const assignmentMembersById = useMemo(
    () =>
      new Map(
        assignmentMembers.map((member) => [member.roomMemberId, member]),
      ),
    [assignmentMembers],
  );
        ...
        <div className="max-h-80 overflow-y-auto">
          ...
          {[...items].sort((a, b) => {
            ...
            const assignedMember = item.assignedTo
              ? assignmentMembersById.get(item.assignedTo)
              : undefined;
            const securedMember = item.securedBy
              ? assignmentMembersById.get(item.securedBy)
              : undefined;
            return (
              <div
                key={item.id}
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    {(assignedMember || securedMember) && (
                      <div className="mt-2 flex flex-wrap gap-1">
                        {assignedMember && (
                          <span className="inline-flex items-center gap-1 rounded-full bg-sky-100 px-2 py-0.5 text-xs font-semibold text-sky-800 dark:bg-sky-900/50 dark:text-sky-200">
                            <span
                              className="h-2 w-2 rounded-full"
                              style={{ backgroundColor: assignedMember.color ?? '#0284c7' }}
                            />
                            担当: {assignedMember.displayName}
                          </span>
                        )}
                        {securedMember && (
                          <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-semibold text-emerald-800 dark:bg-emerald-900/50 dark:text-emerald-200">
                            <span
                              className="h-2 w-2 rounded-full"
                              style={{ backgroundColor: securedMember.color ?? '#059669' }}
                            />
                            確保: {securedMember.displayName}
                          </span>
                        )}
                      </div>
                    )}

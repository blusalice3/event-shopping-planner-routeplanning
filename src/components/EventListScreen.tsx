    ...
    <div className="space-y-4 animate-fade-in">
      ...
      {menuVisibleFor && (
        <div 
          ...
          onClick={() => { setMenuVisibleFor(null); }}
        >
          <div 
          ...
          >
            <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
              <h3 className="text-sm font-semibold text-slate-800 dark:text-white truncate">{menuVisibleFor}</h3>
            </div>
            <div className="py-2">
              {onUpdate && (
                <button 
                    onClick={(e) => { e.stopPropagation(); onUpdate(menuVisibleFor); setMenuVisibleFor(null); }}
              ...
              {onRename && (
                <button 
                    onClick={(e) => { e.stopPropagation(); onRename(menuVisibleFor); setMenuVisibleFor(null); }}
                    className="flex items-center space-x-3 w-full px-4 py-3 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
                >
              ...
              {onImportMap && (
                <button 
                    onClick={(e) => { e.stopPropagation(); onImportMap(menuVisibleFor); setMenuVisibleFor(null); }}
                    className="flex items-center space-x-3 w-full px-4 py-3 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
                >
                    <span className="text-lg">🗺️</span>
              ...
              <button 
                  onClick={(e) => { e.stopPropagation(); onExport(menuVisibleFor); setMenuVisibleFor(null); }}
                  className="flex items-center space-x-3 w-full px-4 py-3 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
              >
              ...
              <div className="border-t border-slate-200 dark:border-slate-700 my-2" />
              <button 
                  onClick={(e) => { e.stopPropagation(); handleDelete(menuVisibleFor); }}

...
describe('App executeModeItems commit integration', () => {
  it('keeps setExecuteModeItems behind the ref-backed commit helper', () => {
    ...
    const helperIndex = source.indexOf('const commitExecuteModeItems = useCallback');
    ...
    expect(source).not.toContain('executeModeItemsRef.current = executeModeItems');
  });

  it('routes persistence loading and updater paths through committed helpers', () => {
    const source = appSource();

    expect(source).toContain('setExecuteModeItems: setExecuteModeItemsCommitted');
    ...
    expect(source).toContain('commitExecuteModeItemsForEvent(activeEventName, result.executeModeItems)');
  });
  ...
  it('keeps candidate/execute normal add, delete, and reorder paths on the ref-backed updater', () => {
    ...
    expect(dragReorderHandler).toContain('executeModeItemsRef.current[activeEventName] || {}');
    expect(verticalReorderHandler).toContain('executeModeItemsRef.current[activeEventName] || {}');
    expect(normalAddHandler).toContain('prev[activeEventName] || {}');
    expect(normalRemoveHandler).toContain('prev[activeEventName] || {}');
    expect(dragReorderHandler).not.toContain('executeModeItems[activeEventName] || {}');
    expect(verticalReorderHandler).not.toContain('executeModeItems[activeEventName] || {}');
    expect(normalAddHandler).not.toContain('executeModeItems[activeEventName] || {}');
    expect(normalRemoveHandler).not.toContain('executeModeItems[activeEventName] || {}');
  });
  ...
  it('keeps event delete, rename, and import execute-list writes on the ref-backed updater', () => {
    ...
    expect(deleteHandler).toContain('updateExecuteModeItems((prev) => removeRecordKey(prev, eventName))');
    expect(renameHandler).toContain(
  ...
  it('keeps batch before position insertion in forward order against the same reference', () => {
    const source = appSource();
    const batchHandlerStart = source.indexOf('const handleBatchAddToExecuteListFromMapAtPosition');

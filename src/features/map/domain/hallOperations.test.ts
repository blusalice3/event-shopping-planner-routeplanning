...
describe('hallOperations regressions', () => {
  it('keeps priority groups while merging refreshed hall ids', () => {
    expect(mergeHallOrder(['hall-a:highest', 'hall-a:priority', 'hall-a'], ['hall-a'])).toEqual([
      'hall-a:highest',
      'hall-a:priority',
      'hall-a',
    ]);
  ...
  it('splits global route settings by extracted base hall id', () => {
    ...
    expect(result.mapSettings.hallOrder).toEqual(['hall-a:priority', 'undefined:highest']);
  ...
  it('reorders execute ids by hall priority groups and visit-list order', () => {
    ...
    const result = reorderExecuteIdsByHallOrder({
      ...
      hallRouteSettings: {
        hallOrder: ['hall-a:priority', 'hall-b', 'hall-a'],
        hallVisitLists: [{ hallId: 'hall-a:priority', itemIds: ['a-priority-1', 'a-priority-2'] }],
      },
    });

    expect(result).toEqual(['a-priority-1', 'a-priority-2', 'b-normal', 'a-normal']);
  });
  ...
  it('combines mapped and mapless route settings for a date', () => {
    const result = getCombinedHallRouteSettingsForDate({
      ...
      hallRouteSettings: {
        event: {
          map: { hallOrder: ['hall-a'], hallVisitLists: [{ hallId: 'hall-a', itemIds: ['a1'] }] },

...
export function useAssignment(
  ...
  const assignItemAction = useCallback(
    async (localItemId: string, targetJerseyNumber: number | null) => {
      if (!supabase || !activeRoom || !userId) return;

      addPendingWrite(localItemId);
      try {
        await assignmentService.assignItem(supabase, activeRoom.id, localItemId, targetJerseyNumber, userId);
  ...
  const bulkAssignItemsAction = useCallback(
    async (localItemIds: string[], targetJerseyNumber: number | null) => {
      if (!supabase || !activeRoom || !userId) return;
      ...
      try {
        await assignmentService.bulkAssignItems(supabase, activeRoom.id, localItemIds, targetJerseyNumber, userId);
        for (const id of localItemIds) {

...
interface AutoAssignmentDialogProps {
  ...
  onApply: (assignments: { jerseyNumber: number; itemIds: string[] }[], clearItemIds: string[]) => Promise<void>;
  onClose: () => void;
}
...
const AutoAssignmentDialog: React.FC<AutoAssignmentDialogProps> = ({
  ...
  // 現在の割り振りからmanualAssignmentsを初期化（背番号ベース）
  const initialManualAssignments = useMemo(() => {
    ...
    for (const space of spaceGroups) {
      ...
      const jerseyNums = new Set(assignedItems.map((item) => item.assignedTo!));
      if (jerseyNums.size === 1 && assignedItems.length === space.itemIds.length) {
  ...
  // 手動割り当てstate: spaceKey → jerseyNumber文字列（現在の割り振りで初期化）
  ...
  const manualSummary = useMemo(() => {
    const summary = new Map<number, { spaceCount: number; itemCount: number }>();
    for (const m of members) {
      summary.set(m.jerseyNumber, { spaceCount: 0, itemCount: 0 });
    }
    for (const space of spaceGroups) {
      const jerseyStr = manualAssignments[space.spaceKey];
      if (jerseyStr) {
        const jerseyNum = Number(jerseyStr);
        if (summary.has(jerseyNum)) {
          const s = summary.get(jerseyNum)!;
          s.spaceCount++;
  ...
  const handleSpaceAssign = useCallback((spaceKey: string, jerseyNumber: number) => {
    const jerseyStr = String(jerseyNumber);
    setManualAssignments((prev) => {
      if (prev[spaceKey] === jerseyStr) {
        const next = { ...prev };
        delete next[spaceKey];
        return next;
      }
      return { ...prev, [spaceKey]: jerseyStr };
    });
  }, []);

  // ブロック一括割り当て
  const handleBulkBlockAssign = useCallback((jerseyNumber: number) => {
    const jerseyStr = String(jerseyNumber);
    setManualAssignments((prev) => {
      const next = { ...prev };
  ...
  // ホール一括割り当て
  const handleBulkHallAssign = useCallback((jerseyNumber: number) => {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      for (const space of spaceGroups) {
        ...
        if (jerseyStr) {
          const jerseyNum = Number(jerseyStr);
          const existing = grouped.get(jerseyNum) ?? [];
          existing.push(...space.itemIds);
          grouped.set(jerseyNum, existing);
        } else {
          clearItemIds.push(...space.itemIds);
        }
      }
      await onApply(
        Array.from(grouped.entries()).map(([jerseyNumber, itemIds]) => ({ jerseyNumber, itemIds })),
        clearItemIds,
      );
      onClose();
    } catch (err) {
          ...
          {members.length >= 2 && unpurchasedItems.length > 0 && (
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-1.5 px-1">
                {members.map((m) => {
                  const s = manualSummary.get(m.jerseyNumber);
                  return (
                    <div key={m.jerseyNumber} className="flex items-center gap-1 text-[10px] text-slate-500 dark:text-slate-400">
                      <div
                      ...
                      <span>#{m.jerseyNumber} {s?.spaceCount ?? 0}SP/{s?.itemCount ?? 0}件</span>
                    </div>
                  );
                })}
              </div>
                    ...
                    {bulkAssignTarget === 'block' && (
                      <div className="absolute right-0 top-full mt-1 z-50 bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-slate-300 dark:border-slate-600 p-2 flex gap-1">
                        {members.map((m) => (
                          <button
                            key={m.jerseyNumber}
                            onClick={() => handleBulkBlockAssign(m.jerseyNumber)}
                        ...
                        {bulkAssignTarget === 'hall' && (
                          <div className="absolute right-0 top-full mt-1 z-50 bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-slate-300 dark:border-slate-600 p-2 flex gap-1">
                            {members.map((m) => (
                              <button
                                key={m.userId}
                                onClick={() => handleBulkHallAssign(m.jerseyNumber)}
                                className="w-7 h-7 rounded-full flex items-center justify-center text-white text-[10px] font-bold hover:scale-110 transition-transform"
                                style={{ backgroundColor: m.color }}
                                title={m.displayName}
                              >

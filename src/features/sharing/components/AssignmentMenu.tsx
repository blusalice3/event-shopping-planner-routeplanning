        ...
        <div className="py-1">
          ...
          {members.map((member) => (
            <button
              ...
              onClick={() => onAssign(member.jerseyNumber)}
              className={`flex items-center space-x-2 w-full px-3 py-2 text-sm hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors ${
                currentAssignedTo === String(member.jerseyNumber) ? 'bg-blue-50 dark:bg-blue-900/30' : ''
              }`}
            >
              <div
              ...
              {currentAssignedTo === String(member.jerseyNumber) && (
                <span className="ml-auto text-blue-500 text-xs">現在</span>
              )}
            </button>
          ))}

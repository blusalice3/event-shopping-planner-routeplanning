...
interface BulkTransferDialogProps {
  ...
  onTransfer: (itemIds: string[], targetJerseyNumber: number) => Promise<void>;
  onClose: () => void;
}
...
const BulkTransferDialog: React.FC<BulkTransferDialogProps> = ({
  ...
  const myJerseyStr = currentJerseyNumber != null ? String(currentJerseyNumber) : null;

  const unpurchasedItems = useMemo(
    () =>
      items.filter(
        (item) =>
          (!item.assignedTo || item.assignedTo === myJerseyStr) &&
  ...
  const handleTransfer = async () => {
    if (selectedJerseyNumber == null || unpurchasedItems.length === 0) return;
    setIsLoading(true);
    setError('');
    try {
      await onTransfer(
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="flex justify-end space-x-3">
            ...
            <button
              ...
              disabled={isLoading || selectedJerseyNumber == null || unpurchasedItems.length === 0}

...
/** 単一アイテムの担当者割り当て（背番号ベース） */
export async function assignItem(
  supabase: SupabaseDB,
  roomId: string,
  localItemId: string,
  targetJerseyNumber: number | null,
  assignedBy: string,
): Promise<void> {
  const assignedToValue = targetJerseyNumber != null ? String(targetJerseyNumber) : null;
  ...
  await supabase.from('activity_log').insert({
    ...
    user_id: assignedBy,
    action: 'assign',
    payload: { local_item_id: localItemId, assigned_to: assignedToValue },
  });
}

/** 複数アイテムの一括担当者割り当て（投げつけ）。targetJerseyNumber=nullで割り当てクリア */
export async function bulkAssignItems(
  ...
  await supabase.from('activity_log').insert({
    ...
    user_id: assignedBy,
    action: 'bulk_assign',
    payload: { item_count: localItemIds.length, assigned_to: assignedToValue },
  });
}

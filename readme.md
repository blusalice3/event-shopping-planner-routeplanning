...
- **CSVファイルからインポート**: ローカルのCSVファイルをアップロード
  ...
  - CSVエクスポート時に保存された配置情報（実行列/候補リスト）がある場合は、インポート時に復元される
...
- **範囲選択**: 起点と終点を設定して範囲内のアイテムを一括で選択/選択解除
  - チェーン表示で範囲を視覚的に確認可能
  - 範囲内のアイテムを一括でチェック/チェック解除できる

- **警告タグ表示**: アイテムに以下の条件が該当する場合、視覚的な警告タグを表示
  - サークル名が重複している場合：「複数種」タグを表示
  - 備考欄に「優先」が含まれる場合：「優先」タグを表示
  - 備考欄に「委託無」が含まれる場合：「委託無」タグを表示
...
- **複数選択**: 複数のアイテムを選択して一括操作
  - チェックボックスで個別選択
  - 範囲選択で一括選択（起点と終点を設定してチェーン表示で範囲を確認）
- **並び替え**: 選択したアイテムをナンバー順にソート（昇順・降順切り替え可能）
- **ブロックソート**: ブロック名で候補リストをソート（昇順・降順切り替え可能）
- **ブロックフィルタ**: 候補リストをブロック値でフィルタリングして表示
  - 備考欄に「優先」または「委託無」が含まれるブロックは黄色でハイライト表示
- **ナンバーソート**: 候補リストの選択アイテムをナンバー順にソート（ブロックフィルタ適用時のみ利用可能）
- **ドラッグ&ドロップ**: アイテムをドラッグ&ドロップで並び替え（自動スクロール機能付き）
- **上/下移動ボタン**: アイテムカードの上下ボタンで順序を変更

#### 実行モード

- **実行列のみ表示**: 実行モード表示列に移動したアイテムのみを表示
- **状態別フィルタ**: 巡回順、後回し、遅参、欠席、売切、購入済でフィルタリング
- **ブロックソート**: ブロック名でソート
...
- **状態別ソート**: 購入状態でフィルタリング（巡回順、後回し、遅参、欠席、売切、購入済）
- **ブロックソート**: ブロック名で昇順・降順にソート（編集モード・実行モード両方で利用可能）
- **ナンバーソート**: 候補リストの選択アイテムをナンバー順にソート（一括操作、昇順・降順切り替え可能、ブロックフィルタ適用時のみ利用可能）
...
- **範囲選択**: 起点と終点を設定して範囲内のアイテムを一括で選択/選択解除（チェーン表示で範囲を視覚的に確認）
- **ズーム機能**: 表示倍率を30%〜150%の範囲で調整（30%, 50%, 75%, 100%, 125%, 150%から選択）
- **ダークモード対応**: システム設定に応じて自動切り替え
- **CSVエクスポート**: リストをCSV形式でダウンロード（実行列の順序と配置情報を保持）
  - 配置情報（実行列/候補リスト）も保存され、再インポート時に復元される
- **リスト更新**: スプレッドシートから変更を取得して自動更新（M列から始まる形式を使用）
- **リスト名変更**: 即売会の名称を変更
- **リスト削除**: 不要なリストを削除
- **長押し操作**: タブを500ms長押しで編集モードと実行モードを切り替え
- **警告タグ表示**: サークル名の重複や備考欄の「優先」「委託無」を視覚的に表示
...
2. 右側の候補リストから購入したいアイテムを選択（複数選択可能）
   - チェックボックスで個別選択
   - 起点と終点を設定して範囲選択（チェーン表示で範囲を確認し、範囲内のアイテムを一括で選択/選択解除）
3. ヘッダーの「選択したアイテムを左列に移動」ボタンをクリック
4. 左側の実行モード表示列でアイテムの順序を調整
   - ドラッグ&ドロップで並び替え
   - 上/下移動ボタンで順序を変更
5. 候補リストでブロックフィルタを使用して、特定のブロックのアイテムのみを表示
   - 備考欄に「優先」または「委託無」が含まれるブロックは黄色でハイライト表示
6. ブロックフィルタ適用時、選択したアイテムをナンバー順にソート（昇順・降順切り替え可能）
...
4. エクスポートされたCSVファイルには配置情報（実行列/候補リスト）も含まれており、再インポート時に復元されます
...
- I列: 列の種類（実行列/候補リスト）
- J列: 列内順番
- メタデータ行: `#METADATA,spreadsheetUrl,<URL>`（スプレッドシートURLが保存されている場合）
...
- 列間移動も可能（編集モード時、候補リストと実行モード列の間で移動）
- 複数選択時は、選択されたアイテム全体が移動

#### 範囲選択機能

- 起点と終点を設定して範囲内のアイテムを一括で選択/選択解除
- チェーン表示で範囲を視覚的に確認可能
- 起点の直上または直下のアイテムを選択した場合は範囲選択にならない（個別選択として扱われる）
...
- CSVエクスポート時に配置情報（実行列/候補リスト）も保存され、再インポート時に復元される

#### 警告タグ表示

- サークル名が重複しているアイテムには「複数種」タグを表示
- 備考欄に「優先」が含まれるアイテムには「優先」タグを表示
- 備考欄に「委託無」が含まれるアイテムには「委託無」タグを表示
- 警告タグが表示されるアイテムは、カード右側に斜めのストライプ模様が表示される

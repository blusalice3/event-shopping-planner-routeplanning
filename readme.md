...
- シート名「品目表」のM列～R列、W列、Y列が自動的にインポートされます
...
| 状態   | アイコン       | 背景色               | 説明                               |
| ------ | -------------- | -------------------- | ---------------------------------- |
| 未購入 | 灰色の円       | ブロックごとの色分け | まだ訪問していない状態             |
...
- 通常のブラウザタブでは、セキュリティ仕様によりURLバーを完全非表示にはできません
- URLバーなしで使う場合は、PWAとして「ホーム画面に追加」または「アプリとしてインストール」してください
- インストール後はアプリ起動時に`standalone`表示となり、URLバーが表示されません

### 例（主要ブラウザ）

- **Android Chrome**: メニュー → 「ホーム画面に追加」または「アプリをインストール」
- **iPhone Safari**: 共有メニュー → 「ホーム画面に追加」
- **PC Chrome/Edge**: URLバー右側のインストールアイコン、またはメニュー → 「アプリをインストール」
- **Android Firefox**: ホーム画面追加はできますが、PWA実装差による安定性の問題があるため推奨対象外です

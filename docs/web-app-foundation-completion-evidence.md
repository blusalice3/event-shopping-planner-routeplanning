...
- managed device は Windows 11 managed self-hosted runner 上の通常 browser tab と実 installed PWAを
...
現行 registry の49 operationは operation別 closed schemaで正規化され、unknown/unused/missing field、
...
| `P1-PWA`       | accepted gate、managed multi-client PWA               | 未達。3-stage managed-device drill 未実行                        |
...
5. canonical physical profileの30 samples、3-stage managed-device drill、24時間観測、必要承認を完了する。
